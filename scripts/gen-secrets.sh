...
# 5. 앱/백업 컨테이너에는 공개 CA 인증서만 공유한다.
#    ca.key, 노드 private key, mongodb-keyfile 은 private PKI 볼륨에만 남긴다.
cp "$PKI/ca.pem" "$PUBLIC_PKI/ca.pem"
chmod 444 "$PUBLIC_PKI/ca.pem"
chown -R mongodb:mongodb "$PUBLIC_PKI"

echo "=========================================="
echo "Shared PKI generation completed."
echo "  CA:       $PKI/ca.pem"
echo "  Public CA:$PUBLIC_PKI/ca.pem"
