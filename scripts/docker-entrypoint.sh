...
# --tlsDisabledProtocols: TLS 1.3 전용으로 구버전 TLS(1.0/1.1/1.2) 비활성화
if [ -f /data/db/certs/mongo.pem ] && [ -f /data/db/certs/ca.pem ]; then
  ...
  echo "TLS certificates found. mode=$TLS_MODE (MONGO_TLS_REQUIRED=${MONGO_TLS_REQUIRED:-true}, TLS 1.3 only)"
  set -- "$@" \
    --tlsMode "$TLS_MODE" \
