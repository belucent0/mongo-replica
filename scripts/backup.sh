...
# shellcheck source=/mongodb/mongo-connect.sh
...
#   수동 백업 / 동작 확인 / 외부(external service 등)에서 온디맨드 트리거할 때 사용.
...
while true; do
  if run_backup; then
    ...
    echo "[$(date -Is)] retrying in ${BACKUP_RETRY_DELAY:-30}s"
    sleep "${BACKUP_RETRY_DELAY:-30}"
  fi
done
